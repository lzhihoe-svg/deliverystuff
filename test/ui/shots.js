...
(async () => {
  ...
  async function seed(page) {
    await page.evaluate(() => {
      ...
      const d1 = api.addJob({ tab: 'delivery', category: 'lalamove', note: 'Deliver before 5pm', photos: ['x'], dueAt: Date.now() + 5*3600000 });
      api.addJob({ tab: 'delivery', category: 'lalamove', note: 'Multi angle photos', photos: ['x','y','z'], dueAt: Date.now() + 45*60000 });
      api.addJob({ tab: 'delivery', category: 'bus', note: 'Bus to Kuantan, counter 7', photos: ['x'], dueAt: Date.now() - 30*60000 });
